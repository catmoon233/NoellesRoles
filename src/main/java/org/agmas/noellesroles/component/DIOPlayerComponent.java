...
public class DIOPlayerComponent implements RoleComponent, ServerTickingComponent {
    ...
    public boolean feedOnCorpse(Entity corpse) {
        if (corpse == null || !(corpse instanceof LivingEntity livingCorpse)) return false;
        if (!livingCorpse.isDeadOrDying()) return false;
        if (!(player instanceof ServerPlayer serverPlayer)) return false;
    ...
    public void extendTempLife() {
        ...
        player.addEffect(new MobEffectInstance(
            MobEffects.MOVEMENT_SPEED, tempLifeRemaining, 1, false, false, true
        ));
