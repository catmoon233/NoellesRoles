...
public class LockGameScreen extends Screen {
    private static class LockPickWidget extends TextureWidget {
        ...
        protected void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float f) {
            for(int i = 0, offsetX = 0, curWidth = bodyWidth;
                i < length;
                ++i, offsetX += bodyWidth * pixelSize)
            {
                if(i == length - 1)
    ...
    protected void init() {
        ...
        int lockPickX = lockStartX - (lockPick.getLockPickWidth() + (lockEntity.getLength() - 1) * (lockWidth - 1) - 4) * pixelSize;
        ...
        // 绘制各个锁及锁芯
        for(int i = 0, curWidth = 0, offsetX = 0;
            i < lockEntity.getLength();
            ++i, offsetX += (lockWidth - 1) * pixelSize)
        {
            // 拼接锁
