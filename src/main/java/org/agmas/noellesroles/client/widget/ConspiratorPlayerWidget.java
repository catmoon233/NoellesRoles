package org.agmas.noellesroles.client.widget;

import org.agmas.noellesroles.client.screen.ConspiratorScreen;
import net.minecraft.client.MinecraftClient;
import net.minecraft.client.font.TextRenderer;
import net.minecraft.client.gui.DrawContext;
import net.minecraft.client.gui.PlayerSkinDrawer;
import net.minecraft.client.gui.widget.ButtonWidget;
import net.minecraft.client.network.AbstractClientPlayerEntity;
import net.minecraft.client.render.RenderLayer;
import org.jetbrains.annotations.NotNull;

import java.awt.*;

/**
 * 阴谋家玩家选择 Widget
 * 
 * 显示玩家头像，点击选择目标玩家
 */
public class ConspiratorPlayerWidget extends ButtonWidget {
    
    public final ConspiratorScreen screen;
    public final AbstractClientPlayerEntity player;
    private final int size;
    
    public ConspiratorPlayerWidget(ConspiratorScreen screen, int x, int y, int size, 
                                   @NotNull AbstractClientPlayerEntity player, int index) {
        super(x, y, size, size, player.getName(), 
            (button) -> screen.onPlayerSelected(player.getUuid(), player.getName().getString()), 
            DEFAULT_NARRATION_SUPPLIER);
        this.screen = screen;
        this.player = player;
        this.size = size;
    }
    
    @Override
    protected void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
        // 绘制背景框
        int bgColor = this.isHovered() ? new Color(100, 50, 150, 200).getRGB() : new Color(50, 25, 75, 150).getRGB();
        context.fill(getX() - 2, getY() - 2, getX() + size + 2, getY() + size + 2, bgColor);
        
        // 绘制边框
        int borderColor = this.isHovered() ? new Color(150, 100, 200).getRGB() : new Color(75, 50, 100).getRGB();
        context.drawBorder(getX() - 2, getY() - 2, size + 4, size + 4, borderColor);
        
        // 绘制玩家皮肤头像
        PlayerSkinDrawer.draw(context, player.getSkinTextures().texture(), getX(), getY(), size);
        
        // 高亮效果
        if (this.isHovered()) {
            drawShopSlotHighlight(context, getX(), getY(), 0);
            
            // 绘制玩家名称提示
            TextRenderer textRenderer = MinecraftClient.getInstance().textRenderer;
            int textWidth = textRenderer.getWidth(player.getName());
            context.drawTooltip(textRenderer, player.getName(), 
                getX() + size / 2 - textWidth / 2, getY() - 12);
        }
    }
    
    private void drawShopSlotHighlight(DrawContext context, int x, int y, int z) {
        int color = new Color(150, 100, 200, 100).getRGB();
        context.fillGradient(RenderLayer.getGuiOverlay(), x, y, x + size, y + size, color, color, z);
    }
    
    @Override
    public void drawMessage(DrawContext context, TextRenderer textRenderer, int color) {
        // 不绘制默认消息
    }
}